...
public class MoodLogFakeRepository
    ...
    public List<User> findUsersWhoDidNotVoteToday(long startOfDay, long endOfDay) {
        var voted = memory.values().stream()
    ...
    @Override
    public Optional<MoodLog> findByUserIdAndCreatedAtBetween(Long userId, long start, long end) {
        return Optional.empty();
    }

    @Override
    public List<MoodLog> findAllByUserClientIdAndCreatedAtGreaterThan(Long clientId, long startTimestamp) {
        return List.of();
    }
}
